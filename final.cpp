...
void *Sequencer(void *threadp)
{
	...
	/*
        ...
        // Service_2 = RT_MAX-2	@ 1.5 Hz - 667 ms
        if((seqCnt % 33) == 0) sem_post(&semS2);*/
	
	
	// Servcie_1 = RT_MAX-1	@ 2 Hz - 500 ms
        if((seqCnt % 25) == 0) sem_post(&semS1);
        
	// Service_2 = RT_MAX-2	@ 1.6 Hz - 620 ms   
        if((seqCnt % 31) == 0) sem_post(&semS2);

	// Service_2 = RT_MAX-2	@ 1.72 Hz - 580.65 ms      
...
void *Service_1(void *threadp)
{
	...
	// PRECAPTURED VIDEO
	VideoCapture cap("lane_yellow_left.mp4");
...
void *Service_2(void *threadp)
{
	...
	/*
	pthread_mutex_lock(&lockS2);
	stopS2_sec[S2Cnt] = (int)(stop_time.tv_sec - start_time_val.tv_sec);
	stopS2_msec[S2Cnt] = (int)stop_time.tv_nsec/NSEC_PER_MSEC;
	pthread_mutex_unlock(&lockS2);
	*/
